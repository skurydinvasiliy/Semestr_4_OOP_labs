 ...
 * чтобы вычислить путь от начала до конца, используя алгоритм поиска путей A *.
...
public class AStarApp {
    ...
    private class MapCellHandler implements MouseListener
    {
         ...
         * То, что это, зависит от исходного состояния ячейки, в которой была запущена операция.
         **/
        ...
        public void mousePressed(MouseEvent e)
        {
            ...
            /** Если текущая ячейка проходима, то мы делаем ее непроходимой **/

            /** если это непроходимо, то мы делаем её проходимой. **/
        ...
        /** Если мышь была нажата, это продолжит операцию модификации в новую ячейку. **/
     ...
     * Это вызывается из потока обработчика событий Swing для обеспечения безопасности потока.
     **/
    private void initGUI()
    {
        ...
        /** Используйте GridBagLayout, потому что он фактически учитывает
         *  предпочтительный размер, указанный компонентами, которые он размещает. **/
    ...
    /** Стартует приложение. Вызывается из метода {@link #main}. **/
    private void start()
    {
     ...
     * Реализация довольно медленная; новый объект {@link Map2D} создается и инициализируется из текущего состояния приложения.
     * Затем вызывается указатель пути A *, и, если путь найден, дисплей обновляется, чтобы показать найденный путь.
    ...
    private void findAndShowPath()
    {
        ...
        /** Попробуйте вычислить путь. Если можно вычислить, отметьте все ячейки на пути. **/
